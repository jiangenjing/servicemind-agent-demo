...
> **仓库状态：架构导览与项目文档。** 本文区分代码已经实现的机制与待验证的能力。源码发布不等于在线服务部署：当前没有提供公网真实模型服务，性能和业务收益需要通过实验验证。建议先用 3—5 分钟阅读概览、架构和请求流程，再按需细读复现与评测部分。
...
本仓库提供完整设计导览，源码包、文件校验清单与环境模板见[源码仓库](https://github.com/jiangenjing/servicemind-source-private)。下载并解压源码包后再执行下述步骤；不能只克隆本介绍仓库就启动后端。
