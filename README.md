# ServiceMind｜多 Agent 客服协作与知识检索

**个人 Agent 工程学习与复现项目：从一句用户问题，到意图识别、角色协作、知识检索、工具执行、记忆和评测。**

`Python` · `FastAPI` · `Tool Calling` · `RAG` · `Redis` · `ChromaDB` · `Vue 3`

本项目面向客服与运营场景，重点研究“怎样让 Agent 按可检查的流程处理问题”，而不只是把大模型接到聊天框。ServiceMind 是我的个人展示名称；项目学习基础来自第三方提供的代码，公开部分为重新整理的架构解读、实现核查和复现路线，不宣称全部底层实现由我独立原创。

> **仓库状态：公开架构展示版。** 尚未确认第三方代码的再分发授权，因此不在公开仓库上传付费源码、课程文档和简历模板。本文区分“在本地代码中确认的机制”和“尚待验证的能力”。当前没有提供公网真实模型服务，也没有展示虚构的生产性能或业务收益。
...
公开版可以直接阅读整套设计，不包含第三方源码。下述步骤面向已合法取得源码的学习者；不能只克隆本公开仓库就运行完整后端。
...
3. 在 Python 项目根目录自行创建未跟踪的 `.env`。原 README 提到的 `.env.example` 在本次目录中未找到，不应照抄不存在文件的复制命令。
...
本仓库没有用付费模板里的示例数字充当本人实测指标。正式结果应同时发布数据集版本、样本量、模型与提示词版本、失败案例和运行成本。
...
公开展示由本人整理与维护；底层学习项目来源与作者信息保留在原始材料中。未取得明确公开许可前不再分发第三方源码、原始付费讲义和包装模板，也不为其附加自行声明的开源许可证。
